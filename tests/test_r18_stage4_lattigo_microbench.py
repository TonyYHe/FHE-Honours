...
from orion.core.region_cir_replay import build_r18_stage4_lattigo_microbench
...
def test_r18_stage4_lattigo_microbench_matches_scripts_cir_stats(monkeypatch) -> None:
    _require_lattigo()

    def fail_pack_conv2d(*_args, **_kwargs):
        raise AssertionError("R18 stage4 Lattigo microbench must not call pack_conv2d")

    monkeypatch.setattr(packing, "pack_conv2d", fail_pack_conv2d)

    payload = build_r18_stage4_lattigo_microbench()

    assert payload["status"] == "ok"
    assert payload["network"] == "R18"
    assert payload["family"] == "stage4_same"
    ...
    assert payload["prepack_execution"] == "plaintext-prepacked compact source; scripts/cir prepack cost included in stats"
