...
def _require_haloed() -> None:
    ...
    script_paths = (
        HALOED_ROOT / "scripts/cir/model_axis_actual_kernel_counts.py",
        HALOED_ROOT / "src/scripts/cir/model_axis_actual_kernel_counts.py",
    )
    if not any(path.exists() for path in script_paths):
        pytest.skip("local HaloED checkout does not expose scripts.cir")
...
def test_haloed_direct_inter_group_plan_bridges_into_orion_python_backend() -> None:
    ...
    shadowed_modules = {
        ...
        if name == "scripts" or name.startswith("scripts.") or name == "haloed" or name.startswith("haloed.")
    }
    try:
