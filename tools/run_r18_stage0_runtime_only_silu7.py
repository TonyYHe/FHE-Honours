...
DEFAULT_OUT = Path("/run/media/anakano/7TB/haloed-cache/orion_runtime_only/orion_r18_stage0_runtime_only_silu7.json")
...
def _build_config() -> dict:
    return {
        ...
        "orion": {
            ...
            "debug": False,
            "io_mode": "none",
            "experimental_region_first": "r18_tiny_e2e_probe_precompile_stage0",
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compile-once runtime-only R18 stage0-only SiLU7 probe.")
    parser.add_argument("--out", type=Path, default=DEFAULT_OUT)
    parser.add_argument("--warmup-seed", type=int, default=1)
    parser.add_argument("--measured-seeds", type=int, nargs="*", default=[2])
    ...
    payload = {
        "status": "started",
        "step": "init",
        "mode": "runtime_only_after_precompile_stage0",
        ...
        "activation": {
            ...
            "selected_stage_filter": ["stage1"],
        },
        "warmup": {},
        "measured_runs": [],
        "claim": {
            "publishable": False,
            "reason": "stage0-only probe with dense fallback and stem ReLU bypass; used only to verify compile/runtime separation",
        },
    }
    out_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
