...
DEFAULT_OUT = Path("/tmp/orion_u22_unique_nodes_lattigo_vs_orion.json")
...
def _init_scheme(*, dataset: str) -> None:
    config = {
        "ckks_params": {
...
def _prepared_dag(*, dataset: str, provider: bool) -> tuple[NetworkDAG, dict[str, Any]]:
    torch.manual_seed(0)
...
def _validate_case(*, dataset: str, node_name: str) -> dict[str, Any]:
    _init_scheme(dataset=str(dataset))
    try:
        dense_dag, _dense_attach = _prepared_dag(dataset=str(dataset), provider=False)
        provider_dag, provider_attach = _prepared_dag(dataset=str(dataset), provider=True)
        ...
        runtime = getattr(provider, "region_runtime")
        torch.manual_seed(abs(hash((str(dataset), str(node_name), "lattigo"))) % (2**31))
        x = torch.randn(tuple(int(v) for v in dense.input_shape[1:]), dtype=torch.float32)
        source = _encode_input(dense, x)

        dense_out = _decode_output(dense, dense(source))
        provider_out = _decode_output(provider, provider(source))
        clear_reference = F.conv_transpose2d(
            x.unsqueeze(0),
            dense.on_weight.detach().to(dtype=torch.float32),
            dense.on_bias.detach().to(dtype=torch.float32) if getattr(dense, "on_bias", None) is not None else None,
            stride=tuple(int(v) for v in dense.stride),
            padding=tuple(int(v) for v in dense.padding),
            output_padding=tuple(int(v) for v in dense.output_padding),
            groups=int(dense.groups),
            dilation=tuple(int(v) for v in dense.dilation),
        )[0]
        ...
        return {
            ...
            "provider_attach_audit": dict(provider_attach),
            "lattigo_new_kernel_vs_orion_dense_ckks": _metrics(dense_out, provider_out),
            "dense_ckks_vs_reference": _metrics(clear_reference, dense_out),
            "new_kernel_ckks_vs_reference": _metrics(clear_reference, provider_out),
        }
    finally:
        _cleanup_scheme()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run local Lattigo-only U22 unique-node comparisons: new kernel vs Orion dense CKKS.")
    ...
    payload = {
        "status": "running",
        "scope": "u22_unique_nodes_lattigo_vs_orion",
        "rows": rows,
    }
    ...
    for dataset in args.datasets:
        for node_name in args.nodes:
            row = _validate_case(dataset=str(dataset), node_name=str(node_name))
            rows.append(row)
